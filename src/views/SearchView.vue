        ...
        <div class="search-page">
            ...
            <div class="date-section">
                ...
                <div class="desktop-picker">
                    <n-date-picker v-model:value="dateRange" type="daterange" clearable
                        :is-date-disabled="disableFutureDate" placement="bottom-start" />
                </div>

                <!-- 移动端双选择器 -->
                <div class="mobile-pickers">
                    <n-flex>
                        <n-date-picker v-model:value="startDate" type="date" clearable style="min-width: 360px;"
                            :is-date-disabled="disableFutureDate" placement="bottom-start" placeholder="开始日期" />
                        <n-date-picker v-model:value="endDate" type="date" clearable style="min-width: 360px;"
                            :is-date-disabled="disableFutureDate" placement="bottom-start" placeholder="结束日期" />
                    </n-flex>
                </div>
            </div>
            ...
            <div class="history-section">
                ...
                <div class="history-content" :class="{ 'is-collapsed': isCollapsed && shouldCollapse }">
                    <n-flex>
                        <n-tag v-for="(item, index) in searchHistory" :key="index" :bordered="false" size="small" round
                            clickable @click="handleHistoryClick(item)">
                            {{ item }}
                        </n-tag>
                    </n-flex>
                </div>

                <n-button v-if="shouldCollapse" text size="small" class="collapse-button"
